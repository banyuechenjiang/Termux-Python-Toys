...
def print_submenu():
    print("\n--- 请选择要处理的文件类型 ---")
    sub_options = [
        "1. 角色卡", "2. QuickReply", "3. 正则", "4. 酒馆脚本", "5. 世界书", "6. 预设", "7. 主题", "0. 返回主菜单"
    ]
    for opt in sub_options: print(f"  {opt}")
    print()
...
def _perform_action(filepath, data, file_type, base_path, manually_sorted_dirs):
    ...
    moved_to_folder, move_skipped = None, False
    ...
    if should_move:
        ...
        moved_to_folder = target_subfolder_name
    elif MOVE_FILES and (is_in_manual_dir or is_in_preset_dir): move_skipped = True
        
    return True, os.path.basename(current_path), moved_to_folder, move_skipped, log_entries
...
def process_directory(base_path, specific_type=None):
    ...
    print("开始规划处理任务...")
    action_plan, checked_files, processed_files = [], collections.defaultdict(list), set()
    ...
    for root, _, files in os.walk(base_path, topdown=True):
        ...
        for filename in files:
            ...
            if not data: continue
            parent_folder_name = os.path.basename(root)
            if parent_folder_name in folder_to_type_map and VALIDATION_MAP[folder_to_type_map[parent_folder_name]](data):
                checked_files[folder_to_type_map[parent_folder_name]].append(filename)
            else:
                types_to_check = [specific_type] if specific_type else PROCESSING_ORDER
                for file_type in types_to_check:
                    if VALIDATION_MAP[file_type](data):
                        action_plan.append({'type': file_type, 'path': filepath, 'data': data}); break
            processed_files.add(filepath)

    run_mode_text = "[预览模式]" if DRY_RUN else "[执行模式]"
    print(f"\n{run_mode_text} " + "━" * 8 + " 开始处理 " + "━" * 8)
    
    all_log_entries, untouched_counts = [], collections.defaultdict(int)
    grouped_actions = collections.defaultdict(list)
    total_processed_count, total_skipped_count = 0, 0
    ...
    for file_type in types_to_execute:
        ...
        if any(file_type not in ["世界书", "预设"] or MOVE_FILES for _ in grouped_actions[file_type]): print(f"\n--- 处理 {file_type} ---")
        for action in grouped_actions[file_type]:
            success, new_name, moved_to, skipped, log_entries = _perform_action(action['path'], action['data'], file_type, base_path, manually_sorted_dirs)
            if success:
                total_processed_count += 1
                if skipped: total_skipped_count += 1
                ...
                if log_entries:
                    ...
                    print(log_msg)
                else: untouched_counts[file_type] += 1
    ...
    if not specific_type:
        print("\n[校验结果]")
        if any(checked_files.values()):
            for ftype, files in checked_files.items(): print(f"  - {ftype}: {len(files)} 个文件已在正确分类中。")
        else: print("  - 未检查到已正确分类的文件。")
    
    print("\n[处理结果]")
    if total_processed_count > 0:
        if len(all_log_entries) > 0:
            action_verb = "将会" if DRY_RUN else "已对"
            print(f"  - {action_verb} {len(all_log_entries)} 个操作(重命名/移动)进行处理。")
        if any(untouched_counts.values()):
            for ftype, count in untouched_counts.items(): print(f"  - {ftype}: 识别了 {count} 个文件，但无需操作。")
        if total_skipped_count > 0: print(f"  - 注意: 有 {total_skipped_count} 个文件因位于保护文件夹中而跳过了移动。")
    elif not any(checked_files.values()): print("  - 未找到符合条件的文件。")
    else: print("  - 所有已识别文件均已在正确位置，无需处理。")
...
def undo_from_log():
    ...
    if not os.path.isdir(logs_dir) or not os.listdir(logs_dir):
        print("错误: 未找到 'logs' 文件夹或其中没有任何日志文件。"); return
    ...
    if GUI_AVAILABLE:
        ...
        if not log_files: print("错误: 'logs' 文件夹中没有找到日志文件。"); return
        print("\n请选择要恢复的日志文件:")
        for i, f in enumerate(log_files): print(f"  {i+1}. {f}")
        try:
            choice = int(input(f"请输入选项 (1-{len(log_files)}): ")) - 1
            if 0 <= choice < len(log_files): log_path = os.path.join(logs_dir, log_files[choice])
        except (ValueError, IndexError): log_path = None
    
    if not log_path: print("未选择日志文件，操作取消。"); return
    try:
        with open(log_path, 'r', encoding='utf-8') as f: actions = json.load(f)
    except (json.JSONDecodeError, FileNotFoundError): print(f"错误: 无法读取或解析日志文件 '{log_path}'。"); return
    ...
    for action in actions:
        try:
            ...
            if op_type == "rename":
                print(f"撤销重命名: {os.path.basename(new)} -> {os.path.basename(original)}"); os.rename(new, original)
            elif op_type == "move":
                print(f"撤销移动: {new} -> {original}"); os.makedirs(os.path.dirname(original), exist_ok=True); shutil.move(new, original)
            success_count += 1
        except FileNotFoundError: print(f"  - 失败: 文件 '{new}' 不存在，可能已被移动或删除。"); fail_count += 1
        except Exception as e: print(f"  - 失败: 发生未知错误: {e}"); fail_count += 1
            
    print(f"\n撤销完成: {success_count} 个操作成功, {fail_count} 个操作失败。")
    while True:
        delete_choice = input(f"\n是否删除此日志文件 '{os.path.basename(log_path)}'? (y/n): ").lower()
        if delete_choice == 'y':
            try: os.remove(log_path); print(f"日志文件 '{os.path.basename(log_path)}' 已删除。")
...
def get_path_from_user():
    if GUI_AVAILABLE:
        ...
        if not path: print("未选择文件夹，操作取消。"); return None
        return path
    else:
        print("\n未检测到GUI环境，切换到命令行路径选择模式。")
        expanded_paths = [os.path.expanduser(p) for p in COMMON_PATHS]
        print("请选择一个常用路径或手动输入：")
        for i, p in enumerate(expanded_paths): print(f"  {i+1}. {p}")
        print(f"  {len(expanded_paths)+1}. 手动输入其他路径")
        while True:
            try:
                choice = int(input(f"请输入选项 (1-{len(expanded_paths)+1}): "))
                if 1 <= choice <= len(expanded_paths): return expanded_paths[choice - 1]
                elif choice == len(expanded_paths) + 1: return os.path.expanduser(input("请输入完整路径: "))
                else: print("无效的选项。")
            except (ValueError, IndexError): print("输入无效，请输入数字。")
...
def main():
    ...
    while True:
        ...
        if choice == '0': print("程序已退出。"); break
        elif choice == 'm': MOVE_FILES = not MOVE_FILES
        elif choice == 'p': DRY_RUN = not DRY_RUN
        elif choice == '3': undo_from_log()
        elif choice in ['1', '2']:
            specific_type = None
            if choice == '2':
                submenu_map = {"1": "角色卡", "2": "QuickReply", "3": "正则", "4": "酒馆脚本", "5": "世界书", "6": "预设", "7": "主题"}
                while True:
                    print_submenu()
                    sub_choice = input("请选择类型 (0-7): ")
                    if sub_choice == '0': break
                    if sub_choice in submenu_map: specific_type = submenu_map[sub_choice]; break
                    else: print("无效的选项。")
                if not specific_type: continue
            
            path = get_path_from_user()
            if not path: continue
            if not os.path.isdir(path): print(f"错误：路径 '{path}' 不存在或不是一个文件夹。"); continue
            if not is_safe_path(path): print("警告：检测到您选择的路径可能为 SillyTavern 用户数据目录，为避免破坏程序结构，已中止操作。"); continue
            process_directory(path, specific_type=specific_type)
        else: print("无效的选项，请重新输入。")
