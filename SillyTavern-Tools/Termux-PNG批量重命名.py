...
class PNGTools:
    ...
    def read_png_metadata_and_classify(png_file_path: str) -> Tuple[Optional[str], int, str]:
        ...
        try:
                    ...
                    if chunk_type_bytes == b'tEXt':
                        ...
                        try:
                            ...
                            if key_str_lower == 'chara':
                                ...
                                try: processed_val_bytes = zlib.decompress(processed_val_bytes)
                                ...
                                except Exception: initial_name_signal = SIGNAL_UNKNOWN_CHARA_PARSE_ERROR
                            elif key_str_lower == 'parameters': sd_text = val_bytes.decode('utf-8', errors='ignore')
                            elif key_str_lower == 'comment': nai_comment = val_bytes.decode('utf-8', errors='ignore')
                            elif key_str_lower == 'software': software_tag = val_bytes.decode('utf-8', errors='ignore')
                            elif key_str_lower == 'source': source_tag = val_bytes.decode('utf-8', errors='ignore')
                        except ValueError: pass
            
            if not has_text_chunks: return None, 0, SIGNAL_PIC_NO_TEXT_PURE_IMAGE
            ...
            is_nai_by_software_and_comment = software_tag and NAI_SOFTWARE_TAG_LOWER in software_tag.lower() and PNGTools._is_nai_parameter_json_content_in_comment(nai_comment)
            is_sd_by_parameters = sd_text and PNGTools._is_sd_parameter_text_content(sd_text)
            is_mixed_by_tags = software_tag and source_tag and NAI_SOFTWARE_TAG_LOWER in software_tag.lower() and ("stable diffusion" in source_tag.lower() or "sd" in source_tag.lower())

            if is_nai_by_software_and_comment and not is_sd_by_parameters: return None, total_text_bytes, SIGNAL_PIC_NAI_PARAMS
            if is_sd_by_parameters: return None, total_text_bytes, SIGNAL_PIC_SD_PARAMS
            if is_nai_by_software_and_comment: return None, total_text_bytes, SIGNAL_PIC_NAI_PARAMS
            if is_mixed_by_tags: return None, total_text_bytes, SIGNAL_PIC_MIXED_SOURCE
            ...
            if isinstance(book_data, dict) and isinstance(book_data.get("entries"), list):
                for entry in book_data["entries"]:
                    if isinstance(entry, dict):
                        content = entry.get("description") or entry.get("content") or entry.get("value") or (entry.get("keys", [""])[0] if isinstance(entry.get("keys"), list) and entry.get("keys") else "")
    ...
    def compare_detailed_chara_data(d1: Optional[DetailedCharaData], d2: Optional[DetailedCharaData]) -> bool:
        ...
        return (d1.norm_name == d2.norm_name and d1.norm_first_mes == d2.norm_first_mes and d1.book_entries_set_str == d2.book_entries_set_str)

    @staticmethod
    def _extract_sort_keys_from_filename(filename_basename: str, p_info: Optional[ProcessedFileInfo]) -> Tuple[int, int, str]:
        if p_info: return (-p_info.file_text_size_kb, -p_info.file_total_size_kb, filename_basename)
        ...
        if text_match:
            try:
                if text_match.group(2) and text_match.group(3): return (-int(text_match.group(3)), -int(text_match.group(2)), filename_basename)
                elif text_match.group(4): return (0, -int(text_match.group(4)), filename_basename)
            except ValueError: pass
...
class PNGFileProcessor:
    ...
    def __init__(self, base_directory: str, verbose_output: bool = False):
        ...
        self.logs_dir = os.path.join(self.script_dir, self.LOGS_DIR_NAME)
    ...
    def _sillytavern_warning_check(self, directory: str):
        ...
        is_potential_st_dir = any(kw in path_l for kw in st_keywords) and any(dkw in path_l for dkw in data_keywords)
        if not is_potential_st_dir and os.path.basename(path_l) in ["characters", "cards"]: is_potential_st_dir = True
        if is_potential_st_dir:
            ...
            print(PNGFileProcessor.SEP_LINE_WARN)
            if self._timed_input("  为确认风险并继续，请输入 'yes': ").strip().lower() != 'yes':
                print("\n操作已由用户取消。程序将退出。")
                sys.exit()
            print("\n风险已确认，即将继续...")
    ...
    def _generate_new_filepath(self, original_filepath: str, base_name_prefix: str, file_size_kb: int, text_size_kb: int, is_param_or_chara_type: bool, target_root_dir: str, current_counter_val: int) -> str:
        core_filename_part = f"{base_name_prefix}-{file_size_kb}KB" + (f"&{text_size_kb}KB" if is_param_or_chara_type else "") + f"-{current_counter_val}"
        ...
        while os.path.exists(new_filepath_candidate) and original_filepath.lower() != new_filepath_candidate.lower():
            conflict_idx += 1
            new_filename_base_no_ext = f"{core_filename_part}_{conflict_idx}"
            new_filepath_candidate = os.path.join(target_root_dir, new_filename_base_no_ext + ".png")
        return new_filepath_candidate

    def _process_single_file(self, original_filepath: str, current_root_for_file: str, current_dir_stats: Dict[str, int], cjk_pending_list: Optional[List[PureImagePendingRenameInfo]] = None) -> Optional[ProcessedFileInfo]:
        payload_str, total_text_chunk_size_bytes, name_or_type_signal = self.tools.read_png_metadata_and_classify(original_filepath)
        is_chara = name_or_type_signal not in [SIGNAL_PIC_NAI_PARAMS, SIGNAL_PIC_SD_PARAMS, SIGNAL_PIC_MIXED_SOURCE, SIGNAL_PIC_NO_TEXT_PURE_IMAGE, SIGNAL_SKIP_OTHER_TEXT_NO_TYPE, SIGNAL_SKIP_FILE_ERROR, SIGNAL_SKIP_UNKNOWN_READ_ERROR, SIGNAL_UNKNOWN_CHARA_NO_NAME, SIGNAL_UNKNOWN_CHARA_PARSE_ERROR] and name_or_type_signal is not None
        ...
        actual_new_filepath = self._generate_new_filepath(original_filepath, base_name_prefix.strip('-'), file_size_kb, text_size_kb, is_param_or_chara_for_naming, target_directory_for_file, self.processed_file_counter)
        ...
        if is_pure and cjk_pending_list is not None and self.tools._contains_chinese_char(os.path.basename(original_filepath)):
            ...
            cjk_pending_list.append(PureImagePendingRenameInfo(original_filepath, core_part_for_cjk, current_root_for_file, file_size_kb))
        ...
        if perform_rename_now:
            if original_filepath.lower() != actual_new_filepath.lower():
                try:
                    ...
                    op_type = "move" if os.path.dirname(original_filepath) != os.path.dirname(actual_new_filepath) else "rename"
                    os.rename(original_filepath, actual_new_filepath)
                    self.operation_log.append({"type": op_type, "original_path": os.path.abspath(original_filepath), "new_path": os.path.abspath(actual_new_filepath)})
                    operation_done = True
                except OSError as e:
                    if self.verbose_output: print(f"  错误: 处理 '{os.path.basename(original_filepath)}' 失败: {e}")
                    current_dir_stats['跳过(重命名/移动失败)'] += 1; self.processed_file_counter -=1; return None
            else: operation_done = True
        
        if operation_done or not perform_rename_now:
            if operation_done and self.verbose_output and original_filepath.lower() != actual_new_filepath.lower(): print(f"  处理: '{os.path.basename(original_filepath)}' -> '{os.path.basename(actual_new_filepath)}'")
            pfi = ProcessedFileInfo(original_filepath=original_filepath, new_filepath=actual_new_filepath if operation_done else original_filepath, initial_meta_name_or_type_signal=name_or_type_signal, chara_json_str=payload_str if is_chara else None, is_character_card=is_chara, is_sd_params_pic=is_sd, is_nai_params_pic=is_nai, is_mixed_source_pic=is_mixed, is_pure_image_pic=is_pure, file_total_size_kb=file_size_kb, file_text_size_kb=text_size_kb)
            if operation_done:
            ...
            return pfi
        self.processed_file_counter -=1; return None

    def _phase1_scan_and_rename(self) -> List[ProcessedFileInfo]:
        self.processed_file_counter = 0; self.all_processed_file_infos = []; self.total_png_files_scanned_phase1 = 0
        overall_stats = collections.defaultdict(int)
        cjk_pending_rename_list: List[PureImagePendingRenameInfo] = []

        print(f"\n{self.SEP_LINE_MAJOR}\n[阶段 1] 文件扫描、分类与初步重命名/移动\n{self.SEP_LINE_MAJOR}")
        if self.verbose_output: print(f"\n  正在扫描主目录: {self.base_directory}")

        for root, dirs, files in os.walk(self.base_directory, topdown=True):
            if self.LOGS_DIR_NAME in dirs: dirs.remove(self.LOGS_DIR_NAME)
            ...
            for filename in files:
                ...
                pfi = self._process_single_file(os.path.join(root, filename), root, current_dir_stats, cjk_pending_rename_list)
                if pfi: self.all_processed_file_infos.append(pfi); processed_in_current_dir_flag = True
            ...
            if current_dir_summary_parts : print(f"  {current_path_display}: 处理 " + ", ".join(current_dir_summary_parts) + "。")
            elif processed_in_current_dir_flag and not current_dir_summary_parts and current_dir_stats['纯图片(中文名待确认)'] > 0: print(f"  {current_path_display}: 有 {current_dir_stats['纯图片(中文名待确认)']} 个纯图片(中文名待确认)。")

        if cjk_pending_rename_list:
            print(f"\n{self.SEP_LINE_MINOR}\n注意: 检测到 {len(cjk_pending_rename_list)} 张纯图片的原始文件名含中文。")
            if self._timed_input("是否统一按标准格式重命名这些中文名纯图片? (y/n, 默认 n): ").strip().lower() == 'y':
                print("  正在重命名用户确认的中文名纯图片...")
                confirmed_cjk_rename_count = 0
                for item_to_rename in cjk_pending_rename_list:
                    pfi_to_update = next((p for p in self.all_processed_file_infos if p.original_filepath == item_to_rename.original_filepath and not any([p.is_character_card, p.is_sd_params_pic, p.is_nai_params_pic, p.is_mixed_source_pic])), None)
                    self.processed_file_counter += 1
                    final_cjk_filepath = self._generate_new_filepath(item_to_rename.original_filepath, "Pic-纯图片", item_to_rename.file_total_size_kb, 0, False, item_to_rename.root_path, self.processed_file_counter)
                    if item_to_rename.original_filepath.lower() != final_cjk_filepath.lower():
                        try:
                            ...
                            self.operation_log.append({"type": "rename", "original_path": os.path.abspath(item_to_rename.original_filepath), "new_path": os.path.abspath(final_cjk_filepath)})
                            if pfi_to_update: pfi_to_update.new_filepath = final_cjk_filepath
                            confirmed_cjk_rename_count +=1; overall_stats['纯图片'] +=1; overall_stats['纯图片(中文名待确认)'] -=1
                            if self.verbose_output: print(f"  处理(中文纯图): '{os.path.basename(item_to_rename.original_filepath)}' -> '{os.path.basename(final_cjk_filepath)}'")
                        except OSError as e:
                            if self.verbose_output: print(f"  错误: 重命名中文纯图 '{os.path.basename(item_to_rename.original_filepath)}' 失败: {e}")
                            self.processed_file_counter -=1
                if confirmed_cjk_rename_count > 0: print(f"  已额外重命名 {confirmed_cjk_rename_count} 个中文名纯图片。")
            else:
        ...
        total_skipped = sum(v for k, v in overall_stats.items() if k.startswith('跳过'))
        if total_skipped > 0: print(f"  因各种原因跳过的文件: {total_skipped}")
        if overall_stats['纯图片(中文名待确认)'] > 0: print(f"    (其中 {overall_stats['纯图片(中文名待确认)']} 个中文名纯图片未按用户选择重命名，保留原名)")
        print(self.SEP_LINE_MINOR)
    ...
    def _phase2_identify_groups_and_compute_data(self, chara_cards_info_list: List[ProcessedFileInfo]) -> Tuple[Dict[str, List[ProcessedFileInfo]], str]:
        report_builder_lines = []; print(f"\n{self.SEP_LINE_MAJOR}")
        phase2_title = "[阶段 2] 角色卡精确重复检测 (内容与视觉均相似)"; report_builder_lines.append(phase2_title); print(phase2_title); print(self.SEP_LINE_MAJOR)
        grouped_by_initial_name: Dict[str, List[ProcessedFileInfo]] = collections.defaultdict(list)
        for p_info in chara_cards_info_list: grouped_by_initial_name[p_info.initial_meta_name_or_type_signal].append(p_info)
        processed_groups_filled_data: Dict[str, List[ProcessedFileInfo]] = {}; exact_duplicates_by_name: Dict[str, List[str]] = collections.defaultdict(list); found_any_exact_duplicates = False
        for name_key, group_files_info_list in grouped_by_initial_name.items():
            if not group_files_info_list: continue
            files_in_group_with_data: List[ProcessedFileInfo] = []; hashes_in_this_group: Dict[str, List[str]] = collections.defaultdict(list); metadata_tuples_in_this_group: Dict[Tuple[Optional[str], str, str], List[str]] = collections.defaultdict(list)
            for p_info_item in group_files_info_list:
                ...
                p_info_item.chara_json_str = None; p_info_item.dhash = self.tools.calculate_dhash(p_info_item.new_filepath)
                ...
                if p_info_item.detailed_chara_data:
                    meta_tuple = (p_info_item.detailed_chara_data.norm_name, p_info_item.detailed_chara_data.norm_first_mes or "", p_info_item.detailed_chara_data.book_entries_set_str or "")
            ...
            for paths_with_same_hash in hashes_in_this_group.values():
                if len(paths_with_same_hash) > 1:
                    ...
                    if len(potential_exact_refs) > 1:
                        ref_p_info = potential_exact_refs[0]; current_exact_set = {ref_p_info.new_filepath}
                        for i in range(1, len(potential_exact_refs)):
                            comp_p_info = potential_exact_refs[i]
                            if self.tools.compare_detailed_chara_data(ref_p_info.detailed_chara_data, comp_p_info.detailed_chara_data) and self.tools.compare_dhashes(ref_p_info.dhash, comp_p_info.dhash):
                                current_exact_set.add(comp_p_info.new_filepath)
                        if len(current_exact_set) > 1: consolidated_duplicates_for_this_name_group.update(current_exact_set)
            for paths_with_same_metadata in metadata_tuples_in_this_group.values():
                if len(paths_with_same_metadata) > 1:
                    ...
                    if len(potential_exact_refs) > 1:
                        ...
                        for p_info_ref in potential_exact_refs: sub_groups[(p_info_ref.file_hash, p_info_ref.dhash)].append(p_info_ref.new_filepath)
                        for group_key, paths_in_sub_group in sub_groups.items():
                            if len(paths_in_sub_group) > 1: consolidated_duplicates_for_this_name_group.update(paths_in_sub_group)
            if consolidated_duplicates_for_this_name_group: exact_duplicates_by_name[name_key].extend(list(consolidated_duplicates_for_this_name_group)); found_any_exact_duplicates = True
        print(f"\n{self.SEP_LINE_MINOR}"); report_builder_lines.append(f"--- {phase2_title.split('] ')[1]} ---"); print(report_builder_lines[-1]); print(self.SEP_LINE_DOT)
        if not found_any_exact_duplicates: no_duplicates_msg = "  未检测到内容、元数据与视觉均完全相同的精确重复角色卡。"; report_builder_lines.append(no_duplicates_msg); print(no_duplicates_msg)
        else:
            detection_msg = "  检测到以下标识名下存在内容、元数据与视觉均精确重复的文件:"; report_builder_lines.append(detection_msg); print(detection_msg)
            for name_identifier, duplicate_paths_raw_list in sorted(exact_duplicates_by_name.items()):
                ...
                p_info_objects_for_sorting = [self._get_p_info_by_abs_path(fp_abs, processed_groups_filled_data) for fp_abs in unique_abs_paths]
                p_info_objects_for_sorting = [p for p in p_info_objects_for_sorting if p]
                sorted_duplicate_paths_abs = sorted([p_info.new_filepath for p_info in p_info_objects_for_sorting], key=lambda p_abs: self.tools._extract_sort_keys_from_filename(os.path.basename(p_abs), next((p_obj for p_obj in p_info_objects_for_sorting if p_obj.new_filepath == p_abs), None)))
                if len(sorted_duplicate_paths_abs) > 1:
                    group_header = f"\n  {name_identifier}: (以下为重复项，建议保留第一个)"; report_builder_lines.append(group_header); print(group_header)
                    for fp_abs_path in sorted_duplicate_paths_abs:
                        relative_path_display = os.path.relpath(fp_abs_path, self.base_directory); report_line = f"    - {relative_path_display}"; report_builder_lines.append(report_line); print(report_line)
        print(self.SEP_LINE_MINOR); return processed_groups_filled_data, "\n".join(report_builder_lines)
    ...
    def _phase3_detailed_comparison_report(self, processed_groups_with_data: Dict[str, List[ProcessedFileInfo]]) -> Tuple[Dict[str, Set[str]], str]:
        report_builder_lines = []; print(f"\n{self.SEP_LINE_MAJOR}"); phase3_title = "[阶段 3] 视觉相似性检测 (元数据不同)"; report_builder_lines.append(phase3_title); print(phase3_title); print(self.SEP_LINE_MAJOR)
        potential_similar_items_by_name: Dict[str, Set[str]] = collections.defaultdict(set); found_any_potential_similarity_in_phase3 = False
        for name_identifier, group_p_info_list in processed_groups_with_data.items():
            ...
            for i in range(len(group_p_info_list)):
                for j in range(i + 1, len(group_p_info_list)):
                    ...
                    if not (p_info1.is_character_card and p_info2.is_character_card and p_info1.file_hash and p_info2.file_hash and p_info1.detailed_chara_data and p_info2.detailed_chara_data and p_info1.dhash and p_info2.dhash): continue
                    if p_info1.file_hash == p_info2.file_hash or self.tools.compare_detailed_chara_data(p_info1.detailed_chara_data, p_info2.detailed_chara_data): continue 
                    if self.tools.compare_dhashes(p_info1.dhash, p_info2.dhash):
                        potential_similar_items_by_name[name_identifier].add(p_info1.new_filepath); potential_similar_items_by_name[name_identifier].add(p_info2.new_filepath); found_any_potential_similarity_in_phase3 = True
        print(f"\n{self.SEP_LINE_MINOR}"); report_builder_lines.append(f"--- {phase3_title.split('] ')[1]} ---"); print(report_builder_lines[-1]); print(self.SEP_LINE_DOT)
        if not found_any_potential_similarity_in_phase3: no_sim_msg = "  未检测到视觉相似 (但元数据不同) 的角色卡。"; report_builder_lines.append(no_sim_msg); print(no_sim_msg)
        else:
            sim_found_msg = "  检测到以下标识名下可能存在视觉相似但元数据不同的文件:"; report_builder_lines.append(sim_found_msg); print(sim_found_msg)
            for name_id, filepaths_abs_set in sorted(potential_similar_items_by_name.items()):
                if len(filepaths_abs_set) > 1:
                    p_info_objects_for_sorting = [self._get_p_info_by_abs_path(fp_abs, processed_groups_with_data) for fp_abs in list(filepaths_abs_set)]
                    p_info_objects_for_sorting = [p for p in p_info_objects_for_sorting if p]
                    sorted_filepaths_abs = sorted([p_info.new_filepath for p_info in p_info_objects_for_sorting], key=lambda p_abs: self.tools._extract_sort_keys_from_filename(os.path.basename(p_abs), next((p_obj for p_obj in p_info_objects_for_sorting if p_obj.new_filepath == p_abs), None)))
                    group_header = f"\n  {name_id}: (以下文件视觉相似，但元数据不同)"; report_builder_lines.append(group_header); print(group_header)
                    for fp_abs_path in sorted_filepaths_abs:
                        relative_path_display = os.path.relpath(fp_abs_path, self.base_directory); report_line = f"    - {relative_path_display}"; report_builder_lines.append(report_line); print(report_line)
        print(self.SEP_LINE_MINOR); return potential_similar_items_by_name, "\n".join(report_builder_lines)

    def _phase4_generate_fine_grained_report(self, all_processed_data_groups: Dict[str, List[ProcessedFileInfo]]):
        print(f"\n{self.SEP_LINE_MAJOR}\n[阶段 4 / 可选] 精细化相似性分类报告 (Type A/B/C)\n{self.SEP_LINE_MAJOR}")
        all_character_p_infos: List[ProcessedFileInfo] = []
        for group_list in all_processed_data_groups.values(): all_character_p_infos.extend([p for p in group_list if p.is_character_card and p.detailed_chara_data and p.dhash and p.file_hash])
        if len(all_character_p_infos) < 2: print("  角色卡数量不足 (<2) 或数据不完整，无法进行阶段 4 成对比较。此阶段跳过。"); print(self.SEP_LINE_MINOR); return
        report_type_a: Dict[str, List[Tuple[ProcessedFileInfo, ProcessedFileInfo]]] = collections.defaultdict(list); report_type_b: Dict[str, List[Tuple[ProcessedFileInfo, ProcessedFileInfo]]] = collections.defaultdict(list); report_type_c: Dict[str, List[Tuple[ProcessedFileInfo, ProcessedFileInfo]]] = collections.defaultdict(list)
        ...
        for i in range(len(all_character_p_infos)):
            for j in range(i + 1, len(all_character_p_infos)):
                ...
                meta_similar = self.tools.compare_detailed_chara_data(p_info1.detailed_chara_data, p_info2.detailed_chara_data); visual_similar = self.tools.compare_dhashes(p_info1.dhash, p_info2.dhash)
                sort_key1 = self.tools._extract_sort_keys_from_filename(os.path.basename(p_info1.new_filepath), p_info1); sort_key2 = self.tools._extract_sort_keys_from_filename(os.path.basename(p_info2.new_filepath), p_info2)
                ...
                if meta_similar and visual_similar: report_type_a[grouping_name].append(ordered_pair_objects)
                elif meta_similar and not visual_similar: report_type_b[grouping_name].append(ordered_pair_objects)
                elif not meta_similar and visual_similar: report_type_c[grouping_name].append(ordered_pair_objects)

        if not any([report_type_a, report_type_b, report_type_c]): print("  未找到符合阶段 4 Type A/B/C 分类标准的相似对。")
        else:
            def output_sorted_pairs_type(report_dict: Dict[str, List[Tuple[ProcessedFileInfo, ProcessedFileInfo]]], type_name_str: str, type_description: str):
                if not report_dict: return
                print(f"\n{self.SEP_LINE_MINOR}\n--- 类型 {type_name_str}: {type_description} ---\n{self.SEP_LINE_DOT}")
                found_in_type = False
                for name_id, pairs_list_obj in sorted(report_dict.items()): 
                    if not pairs_list_obj: continue
                    found_in_type = True
                    sorted_pairs_list_obj = sorted(pairs_list_obj, key=lambda p_tuple: self.tools._extract_sort_keys_from_filename(os.path.basename(p_tuple[0].new_filepath), p_tuple[0]))
                    print(f"  {name_id}:")
                    for p_obj1, p_obj2 in sorted_pairs_list_obj:
                        p1_rel = os.path.relpath(p_obj1.new_filepath, self.base_directory); p2_rel = os.path.relpath(p_obj2.new_filepath, self.base_directory)
                        print(f"    - {p1_rel}\n    - {p2_rel}\n      " + "-"*20)
                if not found_in_type: print("    此类型下未找到符合项。")
            output_sorted_pairs_type(report_type_a, "A", "元数据与视觉均相似"); output_sorted_pairs_type(report_type_b, "B", "仅元数据相似, 视觉不同"); output_sorted_pairs_type(report_type_c, "C", "仅视觉相似, 元数据不同")
        print(f"\n{self.SEP_LINE_MAJOR}\n--- 阶段 4 精细化报告结束 ---\n{self.SEP_LINE_MAJOR}")

    def run(self):
        method_overall_start_time = time.perf_counter()
        self.total_input_wait_time = 0.0
        print(f"\n{self.SEP_LINE_MAJOR}\n--- 开始处理目录: {self.base_directory} ---\n{self.SEP_LINE_MAJOR}")
        if not PNGFileProcessor._check_termux_storage_permission(self.base_directory): sys.exit("错误: 存储权限不足，无法继续。")
        ...
        if self.total_png_files_scanned_phase1 == 0:
            print(f"\n{self.SEP_LINE_FINAL}\n操作结束：未扫描到任何 PNG 文件。\n{self.SEP_LINE_FINAL}"); self._print_timing_info(method_overall_start_time); return
        if not self.all_processed_file_infos and self.processed_file_counter == 0 and self.total_png_files_scanned_phase1 > 0:
            print(f"\n{self.SEP_LINE_FINAL}\n操作结束：扫描到 {self.total_png_files_scanned_phase1} 个PNG文件，但无文件被处理或分类。\n{self.SEP_LINE_FINAL}"); self._print_timing_info(method_overall_start_time); return
        ...
        if not chara_for_analysis:
            print(f"\n{self.SEP_LINE_FINAL}\n操作部分完成：阶段 1 执行完毕，但无有效角色卡可供后续分析。\n{self.SEP_LINE_FINAL}"); self._print_timing_info(method_overall_start_time); return
        ...
        if "未检测到内容、元数据与视觉均完全相同的精确重复角色卡。" not in report_p2_text and any(line.strip().startswith("-") or (line.strip().startswith("  ") and ":" in line) for line in report_p2_text.splitlines()): clipboard_content_parts.append(report_p2_text)
        if "未检测到视觉相似 (但元数据不同) 的角色卡。" not in report_p3_text and any(line.strip().startswith("-") or (line.strip().startswith("  ") and ":" in line) for line in report_p3_text.splitlines()): clipboard_content_parts.append(report_p3_text)
        final_clipboard_output_text = "\n\n".join(clipboard_content_parts).strip()
        
        print(f"\n{self.SEP_LINE_CLIP}\n剪贴板提示:")
        if final_clipboard_output_text: self.tools._set_clipboard_text_termux(final_clipboard_output_text); print("  阶段 2 和 3 的相关报告摘要已尝试复制到剪贴板。")
        else: print("  阶段 2 和 3 未发现需报告的精确重复或视觉相似项，剪贴板无内容。")
        print(self.SEP_LINE_CLIP)

        print(f"\n--- 主要阶段 (1, 2, 3) 处理完成 ---")
        if len(chara_for_analysis) >=2:
            if self._timed_input("\n是否生成阶段 4 的精细化相似性分类报告 (Type A/B/C)?\n (y/n, 默认 n): ").strip().lower() == 'y': self._phase4_generate_fine_grained_report(data_p2_results)
            else: print("  已跳过阶段 4 精细化报告。")
        else: print("  角色卡数量不足或数据不完整，已自动跳过阶段 4。")
    ...
    def _print_timing_info(self, method_overall_start_time: float):
        ...
        if self.operation_log: self._save_log_file()
        else: print("  本次操作没有执行任何文件系统修改，未生成日志。")

    def _save_log_file(self):
        timestamp = time.strftime("%Y%m%d_%H%M%S")
        log_filename = os.path.join(self.logs_dir, f"op_log_{timestamp}.json")
        try:
            with open(log_filename, 'w', encoding='utf-8') as f:
                json.dump(self.operation_log, f, indent=2, ensure_ascii=False)
            print(f"\n操作已记录到日志文件: {os.path.relpath(log_filename, self.script_dir)}")
        except Exception as e:
            print(f"\n错误：无法保存日志文件: {e}")

def undo_from_log():
    script_dir = os.path.dirname(os.path.abspath(__file__))
    logs_dir = os.path.join(script_dir, PNGFileProcessor.LOGS_DIR_NAME)
    if not os.path.isdir(logs_dir) or not os.listdir(logs_dir):
        print(f"\n错误: 未找到 '{PNGFileProcessor.LOGS_DIR_NAME}' 文件夹或其中没有任何日志文件。")
        return

    log_files = sorted([f for f in os.listdir(logs_dir) if f.lower().endswith(".json")], reverse=True)
    if not log_files:
        print(f"\n错误: '{PNGFileProcessor.LOGS_DIR_NAME}' 文件夹中没有找到 .json 日志文件。")
        return
    ...
    try:
        ...
        if choice_str == '0': print("  操作已取消。"); return
        choice_idx = int(choice_str) - 1
        if 0 <= choice_idx < len(log_files):
            log_path_to_undo = os.path.join(logs_dir, log_files[choice_idx])
        else:
            print("  无效的选项。"); return
    except (ValueError, IndexError):
        print("  输入无效，请输入一个列表中的数字。"); return

    try:
        with open(log_path_to_undo, 'r', encoding='utf-8') as f:
            actions = json.load(f)
    except (json.JSONDecodeError, FileNotFoundError) as e:
        print(f"  错误: 无法读取或解析日志文件 '{os.path.basename(log_path_to_undo)}': {e}"); return

    if not isinstance(actions, list) or not all(isinstance(i, dict) and 'type' in i and 'original_path' in i and 'new_path' in i for i in actions):
        print("  错误: 日志文件内容格式不符合预期。"); return
    
    print(f"\n即将根据日志 '{os.path.basename(log_path_to_undo)}' 撤销 {len(actions)} 个操作。")
    if input("  请确认是否继续? (y/n): ").strip().lower() != 'y':
        print("  操作已由用户取消。"); return
    ...
    for action in reversed(actions):
        try:
            ...
            if not os.path.exists(new):
                print(f"  失败: 源文件 '{new}' 不存在，无法撤销。"); fail_count += 1; continue
            if os.path.exists(original):
                print(f"  失败: 目标路径 '{original}' 已存在文件，为防止覆盖已跳过。"); fail_count += 1; continue
            if op_type == "move": os.makedirs(os.path.dirname(original), exist_ok=True)
            os.rename(new, original)
            print(f"  成功: 撤销 {op_type} '{os.path.basename(new)}' -> '{os.path.basename(original)}'")
            success_count += 1
        except Exception as e:
            print(f"  失败: 发生未知错误: {e}"); fail_count += 1
            
    print(f"\n撤销完成: {success_count} 个操作成功, {fail_count} 个操作失败。")
    while True:
        delete_choice = input(f"\n是否删除此日志文件 '{os.path.basename(log_path_to_undo)}'? (y/n): ").lower()
        if delete_choice == 'y':
            try: os.remove(log_path_to_undo); print(f"  日志文件 '{os.path.basename(log_path_to_undo)}' 已删除。")
            except OSError as e: print(f"  删除日志文件失败: {e}")
            break
        elif delete_choice == 'n': print("  日志文件已保留。"); break
        else: print("  无效的输入，请输入 'y' 或 'n'。")
...
def run_main_processor():
    ...
    verbose_flag_input = input("\n是否输出详细的阶段1文件操作日志 ? (y/n, 默认 n): ").strip().lower() == 'y'
