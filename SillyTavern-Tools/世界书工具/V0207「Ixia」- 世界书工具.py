...
class WorldbookManager:
    ...
    def delete_split_directory() -> Optional[str]:
        ...
        split_directories = [
            item for item in os.listdir(".") if os.path.isdir(item) and item.endswith("-拆分")
        ]
    ...
    def _extract_info(
        self, content: str, fileName: str, relative_folder_path: str, root_folder_name: str, uid: int, displayIndex: int
    ) -> dict:
    ...
    def generate_worldbook(self, output_filename: str = "worldbook.json"):
...
---
        ...
        for folder_path, dirnames, filenames in os.walk(self.root_dir):
            ...
            entries[uid_counter] = self._create_divider_entry(
                uid_counter, display_index, relative_folder_path, current_folder_files, True, folder_order
            )
            folder_stack.append({"path": relative_folder_path, "order": folder_order})
            uid_counter += 1
            display_index += 1
            folder_order += 10
            ...
            for file_name in filenames:
                if file_name.endswith((".txt", ".md", ".yaml", ".yml")):
                    ...
                    info = self._extract_info(
                        content, file_name, relative_folder_path, uploadFolderName, uid_counter, display_index
                    )
                    if info:
                        depth = 4
    ...
    def split_worldbook(self, json_filepath: str, output_file_ext: str = "txt"):
        ...
        try:
            ...
            print(
                f"错误: JSON 文件解析失败，请检查文件 '{json_filepath}' 格式是否正确，可能不是有效的 JSON 文件。"
            )
            return
        ...
        json_filename_without_ext = os.path.splitext(os.path.basename(json_filepath))[0]
        ...
        for entry_id, entry_data in entries.items():
            ...
            self._process_entry(
                entry_data, output_root_dir, output_file_ext, entry_id, processed_entries_count, total_entries
            )
    ...
    def _is_divider_entry(self, entry_data: dict) -> bool:
        ...
        return comment.startswith("--始 ") and comment.endswith("--") or comment.startswith("--") and comment.endswith(" 终--")
    ...
    def _extract_file_name(self, entry_data: dict, output_file_ext: str, entry_id: str) -> str:
    ...
    def _extract_divider_info(self, entry_data: dict) -> Tuple[Optional[str], Optional[List[str]]]:
        comment = entry_data.get("comment", "")
        content = entry_data.get("content", "")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if operation_type == "1":
            ...
            if selected_json:
                output_ext = (
                    input("请选择输出文件扩展名 (txt/md/yaml, 默认为 txt): ").strip().lower() or "txt"
                )
