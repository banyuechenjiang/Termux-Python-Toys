...
class ErbFileViewer:
    def __init__(self, master):
        ...
        self.case_print_only_var = tk.BooleanVar(value=False) # 新增配置: 仅查看 case 和 Print 内容
        self.comment_only_var = tk.BooleanVar(value=False) # 新增配置: 仅查看注释
        ...
        self.paned_window.add(self.left_frame, weight=1) # weight=1 使左侧框架可伸缩

        # --- ERB 文件列表区域 (移动到左侧框架) ---
        self.erb_list_frame = ttk.Frame(self.left_frame)
        self.erb_list_frame.pack(fill=tk.BOTH, expand=True, pady=(0, 5)) # 稍微减少 pady
        ...
        self.load_selected_file_button = tk.Button(
            self.left_frame, text="加载选中文件", command=self.load_selected_erb_file, state=tk.DISABLED)
        self.load_selected_file_button.pack(side=tk.BOTTOM, fill=tk.X, pady=(0, 5)) # 减少 pady
        self.erb_list_listbox.bind('<<ListboxSelect>>', self.on_erb_file_select)


        # --- 右侧框架 (文本显示) ---
        self.right_frame = ttk.Frame(self.paned_window, padding=5)
        self.paned_window.add(self.right_frame, weight=3) # weight=3 使右侧框架更宽

        # --- 主文本区域 (移动到右侧框架) ---
        self.text_panel_frame = ttk.Frame(self.right_frame, borderwidth=2, relief=tk.GROOVE, padding=5)
        self.text_panel_frame.pack(expand=True, fill=tk.BOTH)
        ...
        self.navigation_frame.pack(pady=5, fill=tk.X) # 填充 X 方向
    ...
    def load_erb_file(self, file_path):
        ...
        try:
            ...
            if self.enable_paging_var.get():
                ...
                self.navigation_frame.pack(pady=5, fill=tk.X) # 确保分页导航显示时填充X
            else:
    ...
    def open_config_window(self):
        ...
        config_options_area.pack(fill=tk.BOTH, padx=10, pady=10, expand=True) # 允许配置区域扩展

        # --- 配置分组 ---
        view_group = ttk.LabelFrame(config_options_area, text="查看选项")
        view_group.pack(fill=tk.X, pady=5)

        filter_group = ttk.LabelFrame(config_options_area, text="内容过滤")
        filter_group.pack(fill=tk.X, pady=5)

        print_group = ttk.LabelFrame(config_options_area, text="PRINT 选项")
        print_group.pack(fill=tk.X, pady=5)

        adjust_group = ttk.LabelFrame(config_options_area, text="调整选项")
        adjust_group.pack(fill=tk.X, pady=5)

        paging_group = ttk.LabelFrame(config_options_area, text="分页选项")
        paging_group.pack(fill=tk.X, pady=5)

        # --- 查看选项 ---
        show_content_check = tk.Checkbutton(view_group, text="显示文件内容", variable=self.show_content_var)
        show_content_check.pack(side=tk.LEFT)
        case_print_only_check = tk.Checkbutton(view_group, text="仅 Case/Print", variable=self.case_print_only_var) # 添加到 查看选项 组
        case_print_only_check.pack(side=tk.LEFT)
        comment_only_check = tk.Checkbutton(view_group, text="仅注释", variable=self.comment_only_var) # 添加到 查看选项 组
        comment_only_check.pack(side=tk.LEFT)


        # --- 内容过滤 ---
        comment_check = tk.Checkbutton(filter_group, text="显示注释", variable=self.comment_var)
        comment_check.pack(side=tk.LEFT)

        variable_check = tk.Checkbutton(filter_group, text="显示变量", variable=self.variable_var)
        variable_check.pack(side=tk.LEFT)

        string_check = tk.Checkbutton(filter_group, text="显示字符串", variable=self.string_var)
        string_check.pack(side=tk.LEFT)

        print_check = tk.Checkbutton(filter_group, text="显示 PRINT 输出", variable=self.print_var)
        print_check.pack(side=tk.LEFT)

        conditional_check = tk.Checkbutton(filter_group, text="显示条件判断", variable=self.conditional_var)
        conditional_check.pack(side=tk.LEFT)

        # --- PRINT 选项 ---
        align_check = tk.Checkbutton(print_group, text="PRINT 左对齐", variable=self.align_var)
        align_check.pack(side=tk.LEFT)

        # --- 调整选项 ---
        indent_label = tk.Label(adjust_group, text="缩进量:")
        indent_label.pack(side=tk.LEFT)
        indent_entry = tk.Entry(adjust_group, width=5, textvariable=self.indent_amount_var)
        indent_entry.pack(side=tk.LEFT)
        self.indent_entry = indent_entry

        # --- 分页选项 ---
        enable_paging_check = tk.Checkbutton(paging_group, text="启用分页", variable=self.enable_paging_var)
        enable_paging_check.pack(side=tk.LEFT)

        page_size_label = tk.Label(paging_group, text="每页行数:")
        page_size_label.pack(side=tk.LEFT)
        page_size_entry = tk.Entry(paging_group, width=5, textvariable=self.page_size_var)
    ...
    def load_page(self, page_num):
        ...
        end_index = min(start_index + page_size, len(self.processed_content_lines))
        page_lines = self.processed_content_lines[start_index:end_index]
        page_content = "\n".join(page_lines)
        self.text_area.delete("1.0", tk.END)
        self.text_area.insert("1.0", page_content)
        self.current_page = page_num
        self.total_pages = (len(self.processed_content_lines) + page_size - 1) // self.get_page_size() if self.get_page_size() > 0 else 1
        self.update_navigation_buttons()
    ...
    def process_file(self):
        ...
        processed_text = self.process_erb(self.cached_content)
        self.processed_content_lines = processed_text.splitlines()
    ...
    def process_erb(self, erb_content):
        ...
        case_print_only = self.case_print_only_var.get() # 获取仅 case/print 配置
        comment_only = self.comment_only_var.get() # 获取仅注释 配置


        variables = {}
        conditional_stack = []

        print_command_pattern = re.compile(r"^(PRINT(?:L|W|V|VL|VW|S|SL|SW|FORM|FORML|FORMW|FORMS|FORMSL|FORMSW)?)\s*(.*)")
        ...
        for line in lines:
            ...
            if comment_only: # 仅注释模式
                if line.startswith(";"):
                    processed_lines.append(line)
                continue #  跳过后续处理，只处理注释行

            if case_print_only: # 仅 case/print 模式
                if line.startswith(("CASE", "PRINT")): #  只处理 CASE 和 PRINT 开头的行
                    processed_lines.append(line) #  原样添加 CASE 和 PRINT 行
                continue # 跳过后续处理，只处理 CASE 和 PRINT 行
            ...
            if line.startswith(";") and show_comments:
                ...
                if show_print:
                    match = print_command_pattern.match(line)
                    if match:
                        command = match.group(1)
                        argument = match.group(2).strip()
                        output = ""

                        if command in ("PRINTV", "PRINTVL", "PRINTVW"):
                            calculated_value = self.calculate_expression(argument, variables)
                            if calculated_value is not None:
                                output = str(calculated_value)
                        elif command in ("PRINTS", "PRINTSL", "PRINTSW"):
                        ...
                        elif command in ("PRINTFORM", "PRINTFORML", "PRINTFORMW", "PRINTFORMS", "PRINTFORMSL", "PRINTFORMSW"):
                            output = argument
                            for var_name, var_value in variables.items():
                                output = re.sub(r"%"+re.escape(var_name)+"%", str(var_value), output)
                                output = re.sub(r"\{"+re.escape(var_name)+r"\}", str(var_value), output)
                        else: # PRINT, PRINTL, PRINTW
                            output = argument

                        if command in ("PRINTL", "PRINTVL", "PRINTSL", "PRINTFORML", "PRINTFORMSL", "PRINTVL", "PRINTFORML", "PRINTFORMSL", "PRINTL"): #Include PRINTL here
                            output += "\n"
                        if align_print:
                            output = output.ljust(80 + indent_amount)
                        processed_lines.append(output)

            elif line.startswith("SIF") and show_conditionals:
                condition = line[3:].strip() # Corrected to remove SIF
